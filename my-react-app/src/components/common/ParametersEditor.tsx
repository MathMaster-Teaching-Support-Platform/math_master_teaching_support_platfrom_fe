...
interface ParametersEditorProps {
  ...
  mathFieldRefs?: {
    ...
    minRefs: React.MutableRefObject<Record<number, HTMLInputElement | null>>;
    maxRefs: React.MutableRefObject<Record<number, HTMLInputElement | null>>;
    constraintRefs: React.MutableRefObject<Record<number, HTMLInputElement | null>>;
  };
}
...
export function ParametersEditor({
  ...
  const defaultMinRefs = useRef<Record<number, HTMLInputElement | null>>({});
  const defaultMaxRefs = useRef<Record<number, HTMLInputElement | null>>({});
  const defaultConstraintRefs = useRef<Record<number, HTMLInputElement | null>>({});

  const nameRefs = mathFieldRefs?.nameRefs ?? defaultNameRefs;
  const minRefs = mathFieldRefs?.minRefs ?? defaultMinRefs;
  const maxRefs = mathFieldRefs?.maxRefs ?? defaultMaxRefs;
