      ...
      <div style={{ overflowX: 'auto' }}>
        ...
        >
          ...
          <tbody>
            {['A', 'B', 'C', 'D'].map(key => {
              const c = clauses[key];
              const isCorrect = c?.correct ?? false;
              return (
                <tr
                  key={key}
                  style={{
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 600 }}>
                    <span
                      style={{
                        padding: '4px 12px',
                        borderRadius: 6,
                        backgroundColor: c?.expected ? '#dcfce7' : '#fee2e2',
                        color: c?.expected ? '#166534' : '#991b1b',
                      }}
                    >
                      {c?.expected ? 'Đúng' : 'Sai'}
                    </span>
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 600 }}>
                    <span
                      style={{
                        padding: '4px 12px',
                        borderRadius: 6,
                        backgroundColor: c?.actual ? '#dcfce7' : '#fee2e2',
                        color: c?.actual ? '#166534' : '#991b1b',
                      }}
                    >
                      {c?.actual ? 'Đúng' : 'Sai'}
                    </span>
          ...
          <tfoot>
            <tr style={{ backgroundColor: '#f9fafb', fontWeight: 600 }}>
              <td colSpan={5} style={{ padding: '16px', borderTop: '2px solid #d1d5db' }}>
