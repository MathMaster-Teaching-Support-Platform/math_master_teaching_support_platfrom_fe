      ...
      <section className="module-page">
        <header className="page-header">
          <div>
            ...
            <p>Quản lý kho câu hỏi theo từng nhóm nội dung và cấu hình chia sẻ cho giáo viên khác.</p>
          </div>
          <button
            className="btn"
            onClick={() => {
        ...
        {isError && (
          <div className="empty">{error instanceof Error ? error.message : 'Không thể tải danh sách ngân hàng câu hỏi'}</div>
        )}
            ...
            {filtered.map((bank) => (
              <article key={bank.id} className="data-card">
                ...
                <div className="row" style={{ flexWrap: 'wrap' }}>
                  <button className="btn secondary" onClick={() => navigate(`/teacher/question-banks/${bank.id}`)}>
                    <BookOpen size={14} />
                    Chi tiết
                  </button>
