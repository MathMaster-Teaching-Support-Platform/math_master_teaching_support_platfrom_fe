...
export function QuestionBankDetailPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useGetQuestionBankById(bankId ?? '', !!bankId);
  const {
    data: questionsData,
  ...
  return (
    <DashboardLayout role="teacher" user={{ name: 'Teacher', avatar: '', role: 'teacher' }} notificationCount={0}>
      <section className="module-page">
        ...
        {isError && <div className="empty">{error instanceof Error ? error.message : 'Không thể tải chi tiết ngân hàng câu hỏi'}</div>}
        {!isLoading && !isError && !bank && <div className="empty">Không tìm thấy ngân hàng câu hỏi.</div>}
        ...
        {!isLoading && !isError && bank && (
          <>
            <article className="hero-card">
              ...
              <div className="row" style={{ justifyContent: 'start', flexWrap: 'wrap', marginTop: 8 }}>
              ...
              <div className="toolbar" style={{ marginTop: 14, border: 0, padding: 0, background: 'transparent' }}>
                <button className="btn secondary" onClick={() => { void refetch(); void refetchQuestions(); }}>
                  <RefreshCw size={14} />
                  Làm mới
                </button>
                <button className="btn secondary" onClick={() => togglePublicMutation.mutate(bank.id)}>
            ...
            {questionsError && (
              <div className="empty">
                {questionsErrorValue instanceof Error ? questionsErrorValue.message : 'Không thể tải danh sách câu hỏi'}
              </div>
            )}
                  ...
                  <tbody>
                    {filteredQuestions.map((question) => (
                      <tr key={question.id}>
                        ...
                        <td>{(question.difficulty && difficultyLabel[question.difficulty]) || question.difficulty || '-'}</td>
            ...
            {totalQuestionPages > 1 && (
              <div className="row" style={{ justifyContent: 'center' }}>
                ...
                <span className="muted">Trang {questionPage + 1} / {totalQuestionPages}</span>
                <button
                  className="btn secondary"
