  ...
  return (
    <div style={{ background: '#000', borderRadius: 12, overflow: 'hidden', width: '100%' }}>
      ...
      <div style={{ padding: '1rem', background: '#fff', borderBottom: '1px solid #e2e8f0' }}>
        <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: 'var(--mod-ink)' }}>{title}</h3>
      </div>
    </div>
  );
};
...
const StudentLessonsTab: React.FC<StudentLessonsTabProps> = ({
  ...
  const curriculumHierarchy = useMemo(() => {
    const groups: Array<{ id: string; title: string; lessons: CourseLessonResponse[]; type: 'SECTION' | 'CHAPTER' | 'OTHER' }> = [];
    
    lessons.forEach(lesson => {
      const section = sections.find(s => s.id === lesson.sectionId);
      const groupId = lesson.sectionId || lesson.chapterId || 'no-group';
      const groupTitle = section?.title || lesson.chapterTitle || (lesson.sectionId ? 'Mục chưa đặt tên' : (lesson.chapterId ? 'Chương chưa đặt tên' : 'Khác'));
  ...
  const renderCurriculum = (isSidebar = false) => {
    if (curriculumHierarchy.length === 0) {
      return isSidebar ? null : <div className="empty"><BookOpen size={32} style={{ opacity: 0.3, marginBottom: 8 }} /><p>Chưa có bài học nào.</p></div>;
    }
      ...
      <div key={group.id}>
        ...
        >
          <div className="section-title-area">
            <ChevronDown size={isSidebar ? 14 : 18} style={{ transform: collapsedSections[group.id] ? 'rotate(-90deg)' : 'none', transition: 'transform 0.2s' }} />
            <span className="section-label" style={isSidebar ? { fontSize: '0.65rem' } : {}}>
              {group.type === 'CHAPTER' ? `Chương ${idx + 1}` : (group.type === 'SECTION' ? `Mục ${idx + 1}` : 'Khác')}
            </span>
            <span className="section-title" style={isSidebar ? { fontSize: '0.82rem' } : {}}>{group.title}</span>
          </div>
          {!isSidebar && (
  ...
  const renderLessonItem = (lesson: CourseLessonResponse, isSidebar = false) => {
    ...
    if (lesson.materials) {
      try {
        materialsList = JSON.parse(lesson.materials);
      } catch (e) {}
    }
            ...
            {!isSidebar && (
              <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginTop: 4 }}>
                ...
                {materialsList.length > 0 && (
                  <span 
                    style={{ fontSize: '0.75rem', color: '#6366f1', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 3 }}
                    onClick={(e) => {
                      e.stopPropagation();
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {materialsList.map((m: any) => (
                ...
                >
                  ...
                  <span>{m.name}</span>
                  <span className="muted" style={{ fontSize: '0.7rem' }}>({(m.size/1024).toFixed(1)} KB)</span>
                </a>
              ))}
            </div>
          </div>
      ...
      {playingLessonId && currentLesson ? (
        ...
        <div className="player-container">
            ...
            <div className="data-card" style={{ padding: '1.25rem' }}>
              ...
              {currentLesson.materials ? (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                  {JSON.parse(currentLesson.materials).map((m: any) => (
                    <a key={m.id} href={m.url} target="_blank" rel="noreferrer" className="sc-cta-btn" style={{ fontSize: '0.82rem', padding: '8px 16px', border: '1px solid #e2e8f0' }}>
                      <FileText size={14} /> {m.name}
                    </a>
                  ))}
                </div>
              ) : (
                <p className="muted" style={{ fontSize: '0.88rem', fontStyle: 'italic' }}>Bài học này chưa có tài liệu đính kèm.</p>
              )}
            </div>
            
            <button className="btn secondary" onClick={() => setPlayingLessonId(null)} style={{ alignSelf: 'flex-start' }}>
              Thoát chế độ học
            </button>
          </div>
          ...
          <div className="data-card player-sidebar">
            <div style={{ padding: '1rem', borderBottom: '1px solid #e2e8f0', background: '#f8fbff' }}>
              <h4 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 800 }}>Nội dung khóa học</h4>
              <p className="muted" style={{ fontSize: '0.75rem', marginTop: 4 }}>{completedCount}/{totalCount} bài học hoàn thành</p>
            </div>
        ...
        <>
          <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
            <div className="stat-card stat-blue">
              ...
              <div><h3>{totalCount}</h3><p>Tổng bài học</p></div>
            </div>
            <div className="stat-card stat-emerald">
              <div className="stat-icon-wrap"><CheckCircle size={20} /></div>
              <div><h3>{completedCount}</h3><p>Đã hoàn thành</p></div>
            </div>
          </div>

          {loadingLessons ? (
            <div className="skeleton-grid">{[1, 2, 3].map(i => <div key={i} className="skeleton-card" />)}</div>
          ) : lessons.length === 0 ? (
            <div className="empty"><BookOpen size={32} style={{ opacity: 0.3, marginBottom: 8 }} /><p>Chưa có bài học nào.</p></div>
          ) : (
            <div className="data-card" style={{ gap: 0, padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '1rem 1.2rem', borderBottom: '1px solid #e8eef8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h4 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 800 }}>Giáo trình học tập</h4>
                <div className="pill-btn active"><Layout size={13} style={{ marginRight: 4 }} /> Phân cấp</div>
              </div>
              {renderCurriculum()}
            </div>
          )}
