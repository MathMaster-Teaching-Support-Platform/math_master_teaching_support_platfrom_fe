...
const CoursePreview: React.FC = () => {
  ...
  const renderStars = (rating: number) => {
    ...
    return (
      <span className="preview-star-row" aria-label={`Đánh giá ${clampedRating} trên 5`}>
        {Array.from({ length: 5 }, (_, index) => (
          <Star
            key={`${rating}-${index}`}
            size={16}
            fill={index < clampedRating ? '#eab308' : 'none'}
            color="#eab308"
          />
        ))}
      </span>
  ...
  const groupedCurriculum = useMemo(() => {
    ...
    if (course.provider === 'CUSTOM') {
      ...
      lessons.forEach((lesson, index) => {
        const groupId = lesson.chapterId || lesson.chapterTitle || 'no-chapter';
  ...
  const handleEnroll = () => {
    ...
    enrollMutation.mutate(id!, {
      onSuccess: (resp) => {
        console.log('[Preview] Enroll success:', resp);
