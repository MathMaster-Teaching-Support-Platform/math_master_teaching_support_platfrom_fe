          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                />
                <Area type="monotone" dataKey="inflow" name="Thu vào" stroke="#22c55e" strokeWidth={2} fillOpacity={1} fill="url(#colorIn)" />
                <Area type="monotone" dataKey="outflow" name="Chi ra" stroke="#ef4444" strokeWidth={2} fillOpacity={1} fill="url(#colorOut)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
                ...
                {summary.categoryBreakdown.map((cat, i) => (
                  <div key={i} className="flex justify-between items-center mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600 truncate w-24" title={cat.categoryName}>
                        {cat.categoryName}
                      </span>
                    </div>
                ...
                transactions.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      <span className={`font-semibold ${t.direction === 'INFLOW' ? 'text-green-600' : 'text-red-600'}`}>
                        {t.direction === 'INFLOW' ? '+' : '-'}{formatVND(t.amount)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-gray-900 font-medium truncate max-w-[200px]" title={t.description || ''}>
                        {t.description || (t.orderCode ? `Đơn hàng #${t.orderCode}` : '—')}
                      </div>
                      {t.userName && (
                        <div className="text-xs text-gray-500 mt-1 truncate max-w-[200px]" title={t.userEmail || ''}>
                          {t.userName}
                        </div>
                      )}
                    </td>
        ...
        {totalElements > 0 && (
          <div className="p-4 border-t border-gray-100 flex justify-between items-center bg-gray-50">
            <span className="text-sm text-gray-500">
              Hiển thị {page * 10 + 1}-{Math.min((page + 1) * 10, totalElements)} trên tổng {totalElements}
            </span>
