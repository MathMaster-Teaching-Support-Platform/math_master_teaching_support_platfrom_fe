...
**Ngày tạo:** 2026-04-11  
**Người tạo:** FE Team  
**Trạng thái:** ✅ Vòng 1 hoàn tất — đã integrate  
...
| --- | ---------------------- | ---- | --------------- | ------------------------------------------------------------------------------ |
| 1   | Admin Identity         | 1    | ✅ Done         | `GET /users/my-info` → `fullName`, `roles[0]`, `avatar`                        |
| 2   | Notification Count     | 1    | ✅ Done         | `GET /v1/notifications/unread-count` → direct `{ unreadCount }`                |
| 3   | Stats Grid             | 1    | ✅ Done         | `GET /admin/dashboard/stats` → `activeEnrollments` (không phải subscriptions)  |
| 4   | Recent Users           | 1    | ✅ Done         | `GET /users/admin/recent` → Spring Page, `ACTIVE`/`INACTIVE` uppercase         |
| 5   | Pending Profiles Count | 1    | ✅ Done (trước) | `GET /teacher-profiles/pending/count` — đã có từ vòng trước                    |
| 6   | Recent Transactions    | 1    | ✅ Done         | `GET /admin/transactions` → Spring Page, chỉ `payos` payment method            |
| 7   | Revenue Chart          | 1    | ✅ Done         | `GET /admin/dashboard/revenue-by-month` → absolute VND, FE tính height%        |
| 8   | Quick Stats            | 1    | ✅ Done         | `GET /admin/dashboard/quick-stats` → `satisfactionRate=-1` → "Chưa có dữ liệu" |
| 9   | System Status          | 1    | ✅ Done         | `GET /admin/system/status` → dynamic services array                            |
...
- **Feature:** Recent Transactions — Nút Chi tiết
- **Vấn đề:** Endpoint `GET /admin/transactions/:id` chưa có. Nút 👁️ hiện không có action.
- **Yêu cầu:** BE cần implement `GET /admin/transactions/:id`
- **Mức độ:** 🟡 Minor (UI render bình thường, chỉ nút Chi tiết chưa hoạt động)

### Issue #2

- **Feature:** Quick Stats — Satisfaction Rate
- **Vấn đề:** BE luôn trả `-1.0` vì chưa có tính năng rating. FE hiển thị "Chưa có dữ liệu".
- **Yêu cầu:** Khi BE có data rating thật, đảm bảo trả số trong [0, 100] — FE sẽ tự render thanh progress.
- **Mức độ:** 🟡 Minor (đã handle gracefully)

### Issue #3

- **Feature:** System Status — Storage
- **Vấn đề:** `usagePercent` luôn null. FE không hiển thị usage bar cho Storage.
- **Yêu cầu:** [UNKNOWN] MinIO expose quota API không? Nếu có, trả `usagePercent: number`.
- **Mức độ:** 🟡 Minor (description text vẫn hiển thị)

---

## Thay đổi đã xác nhận từ Y.md (so với spec FE ban đầu)

| Field FE spec               | Field BE thực tế       | Đã xử lý trong code                    |
| --------------------------- | ---------------------- | -------------------------------------- |
| `name`                      | `fullName`             | ✅ `adminUser?.fullName`               |
| `role` (string)             | `roles` (string[])     | ✅ `roles[0]`                          |
| `{ success: true }`         | `{ code: 1000 }`       | ✅ `applyApiResult` helper             |
| `result.data`               | `result.result`        | ✅ `ApiResponse<T>.result`             |
| `activeSubscriptions`       | `activeEnrollments`    | ✅ Label updated                       |
| `result.users`              | `result.content`       | ✅ Spring Page                         |
| `result.total`              | `result.totalElements` | ✅ (chưa dùng pagination)              |
| `joinedDate`                | `createdDate`          | ✅ `user.createdDate`                  |
| `lastLogin`                 | **không có**           | ✅ Đã xóa cột                          |
| `school`                    | **không có**           | ✅ Đã xóa cột                          |
| `status` lowercase          | `status` UPPER_CASE    | ✅ `.toLowerCase()` khi dùng CSS class |
| `wallet/momo/bank_transfer` | `payos` only           | ✅ Render "💳 PayOS"                   |
| Revenue: giá trị %          | Revenue: VND tuyệt đối | ✅ FE tính `height%` theo max          |
| `satisfactionRate: 96`      | `satisfactionRate: -1` | ✅ Render "Chưa có dữ liệu"            |
